...
// locals
[[block]] struct Projection { matrix: mat4x4<f32>; };
[[block]] struct Dim { vec: vec2<f32>; flat: vec2<f32>; };
...
fn fs_main(in: VertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    let re = dim.vec.x * dim.vec.y / length(Rn * vec2<f32>(dim.vec.y, dim.vec.x)); // distance at edge
