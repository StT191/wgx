...
main_app_closure! {
  LogLevel::Warn,
  WindowAttributes::default().with_inner_size(PhysicalSize::new(1280, 900)),
  init_app,
}

async fn init_app(ctx: &mut AppCtx) -> impl FnMut(&mut AppCtx, &Event) {
  ...
  move |_ctx: &mut AppCtx, event: &Event| match event {
