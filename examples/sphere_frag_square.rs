...
async fn init_app(ctx: &mut AppCtx) -> impl FnMut(&mut AppCtx, &AppEvent) {
  ...
  let vertex_buffer = gx.buffer(BufUse::VERTEX | BufUse::COPY_DST | BufUse::MAP_READ, mesh_size, false);
  let normal_buffer = gx.buffer(BufUse::VERTEX | BufUse::COPY_DST /*| BufUse::MAP_READ*/, mesh_size, false);
  ...
  gx.with_encoder(|encoder| {
    ...
    encoder.with_render_pass(
      ([
        Some(compute_tex.color_attachment(Some(Color::TRANSPARENT))),
        Some(compute_tex_normal.color_attachment(Some(Color::TRANSPARENT))),
      ], None),
      |rpass| {
  ...
  /*vertex_buffer.with_map_sync(&gx, 0..(6*vertex_size), MapMode::Read, |buffer_slice| {

    let mapped = buffer_slice.get_mapped_range();
    let vertices: &[[Vertex;3]] = unsafe { mapped.align_to().1 };
