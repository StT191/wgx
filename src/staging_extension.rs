...
impl StagingBeltExtension for StagingBelt {
  ...
  fn stage(
    ...
    let byte_range = byte_range.map_into(0..target.size()).unwrap();
    self.write_buffer(encoder, target, byte_range.start, BufferSize::new(byte_range.end).unwrap(), gx.device())
  }
